...
import Timer as Tm
import FileScripts_feb as f  # Processes file manipulations
...
global my_project, my_angle, my_title, my_script, str_angles, count_limit, count, pa, pb, my_pensize, L, R, G, B, M, my_length, my_pensize_a, my_length_bk, my_random, my_circle, my_script
...
def make_folder():  # Creates directories for video production; selects audio track to be used; sets watermark(audio track).
    ...
    Lg.logger.info(
        f"{my_script}:The {len(a.i_angle)} angles to be drawn are {formatted_angles}"
    )
    Lg.logger.info(
...
#     au.pick_earth_tone_track()  # Default state is uncommented. Use it to select a randomly selected track in audio_clips module.
    ...
    Lg.logger.info(
        f"{my_script}:Setting up directories and files for video production  @ {Tm.this_time}"
    )
    folder_name = (
        Lg.my_project.replace("[", "")
        .replace("]", "")
        .replace("'", "")
        .replace(".0", "")
    )  # .replace(" ","")
...
def process_thumbs():
    ...
    if count + 1 == count_limit:
        for iterable in range(
            480
        ):  # 360 is default. change as appropriate for durationof the end pause
            f.save_thumb()
        clear_p_pens()
        reset_random_hue_pens()
    #         gc.collect()
...
def produce_video():
    ...
    gc.collect()
    Lg.logger.info(
        f"{my_script}:Starting produce_video script @ {Tm.this_time}........"
    )
    f.set_vid_env()
    Tm.set_time()
    Lg.logger.info(
    ...
    Lg.logger.info(
        f"{my_script}:Total Run Time of the produce_video script is {elapsed_time/3600:.2f} hours or {elapsed_time/60:.2f} minutes"
    )
    Lg.logger.info(
        "*********************************************************************************************************************"
    )
...
def produce_reversing_video():
    ...
    Lg.logger.info(
        f"{my_script}:Total Run Time of the produce_reversing_video script is {elapsed_time/3600:.2f} hours or {elapsed_time/60:.2f} minutes"
    )
    Lg.logger.info(
        "****************************************************************************"
        "*************************************************"
...
#     Lg.logger.info(f"{my_script}:Screenshots deletions completed @ {Tm.this_time}")
    ...
    Lg.logger.info(
        f"{my_script}:Folders and files of {my_script} have been synced and backed up"
    )
    ...
    Lg.logger.info(
        f"{my_script}:All modules of {my_script} have been completed successfully."
    )
    Lg.logger.info(f"{my_script}: Program shutting down @ {Tm.this_time}")
    Lg.logger.info(
        f"{my_script}:Module Total Run Time: {module_elapsed_time/3600:.2f} hours"
    )
    Lg.logger.info(
        ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
    )
...
def report_loop_count():
    if count % 100 == 0 and count < count_limit - 1:
        Lg.logger.info(
            f"{my_script}:Just completed loop # {count} of {my_script} @ {Tm.this_time}"
        )
    elif count == count_limit - 1:
        Lg.logger.info(
            f"{my_script}:Just completed final loop # {count} of {my_script} @ {Tm.this_time}"
        )
    else:
        pass
...
def flower_of_life():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        count_limit = 60  # 765 is default; Adjust as needed for output to fit screen; use lower or higher number for testing
...
def Assurance_mandala():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        for count in range(count_limit):
            ...
            pb.pencolor(
                255 - count % 255, count % 255, count % 255
            )  # Pen magenta_hue_pen  (Pen 2)
            pc.pencolor(
                count % 255, count % 127, my_random - count % 150
            )  # Pen  (Pen 3)
...
def basic_yin_yang():  # **
    ...
    turtle.bgcolor(
        125, 125, 125
    )  # Has to be a neutral shade like grey to contrast the black and white theme.
    for count in range(2):  # Default is 5
        for iterable in range(
            count_limit
        ):  # 1800 is default, duration 10 minutes. Use lower number for testing. The higher the number, the longer the show.
            pa.pensize(10)
...
def basic_yin_yang_extended():  # **
    ...
    for count in range(
        count_limit
    ):  # 600 is default, duration 3.5 minutes. Use lower number for testing. The higher the number, the longer the show.
        pa.pensize(10)
        pa.color(0, 0, 0)  # 0,0,0 is default (Black)
        pa.rt(-my_angle + phi)
        pa.circle(250)
        ...
        Lg.logger.info(
            f"{my_script}:The value of Tm.iterable_time is {Tm.iterable_time}"
        )
...
def breathing_yin_yang():  # **
    ...
    turtle.bgcolor(
        127, 127, 127
    )  # Default is 127-127-127. Has to be a neutral shade like grey to contrast the black and white theme.
    ...
    for count in range(2500):  #
        ...
        if count % 500 == 0:
            Lg.logger.info(
                f"{my_script}:The value of count is {count} @ {Tm.this_time}"
            )
...
def mystical_mandala():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        count_limit = (
            510  # Try for multiples of 255 to sync with the 255 colormode settings
        )
        ...
        Lg.logger.info(
            f"{my_script}:Elapsed Time: {elapsed_time:.2f} seconds, or {elapsed_time / 60:.2f} minutes"
        )
        ...
        #         place_pen() # To be used to centering the mandala if required, default is commented out
        count = 0
        for count in range(count_limit):  # Second pass
            pa.pensize(count / my_pensize)
        ...
        Lg.logger.info("**************************************")
        Lg.logger.info(
            f"{my_script}:{my_script}:Starting third pass @ {Tm.this_time}...."
        )
        ...
        Lg.logger.info(f"{my_script}:The value of hue L is {L}")
        #         place_pen() # To be used to centering the mandala if required, default is commented out
        for count in range(count_limit):  # Third pass
            pa.pensize(count / my_pensize)
        ...
        Lg.logger.info("**************************************")
        Lg.logger.info(
            f"{my_script}:{my_script}:Starting fourth pass @ {Tm.this_time}...."
        )
        ...
        Lg.logger.info(f"{my_script}:The value of hue R is {R}")
        Lg.logger.info(f"{my_script}:The value of hue L is {L}")
        for count in range(count_limit):  # Fourth pass
            pa.pensize(count / my_pensize)
        ...
        Lg.logger.info("**************************************")
        Lg.logger.info(
            f"{my_script}:{my_script}:Starting fifth pass @ {Tm.this_time}...."
        )
        ...
        Lg.logger.info(f"{my_script}:The value of hue L is {L}")
        count = 0
        for count in range(count_limit):  # Fifth pass
            pa.pensize(count / my_pensize)
        ...
        Lg.logger.info("**************************************")
        Lg.logger.info(
            f"{my_script}:{my_script}:Starting sixth pass @ {Tm.this_time}...."
        )
        ...
        Lg.logger.info(f"{my_script}:The value of hue R is {R}")
        Lg.logger.info(f"{my_script}:The value of hue L is {L}")
        for count in range(count_limit):  # Sixth pass
            pa.pensize(count / my_pensize)
...
def simple_mandala():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        count = 0
        count_limit = 1800  # default is 1800
        ...
        for count in range(count_limit):
            ...
            pa.pencolor(
                random.randint(25, 100) + count % 100, count % 255, 255 - count % 255
            )
            ...
            red_hue_pen.pencolor(
                255 - count % 255, random.randint(25, 100) + count % 100, count % 255
            )
            pa.rt(my_angle)
            pa.fd(count / my_length)
        ...
        for count in range(count_limit):
            ...
            pa.pencolor(
                255 - count % 127,
                255 - count % 255,
                random.randint(25, 100) + count % 100,
            )
        ...
        for count in range(count_limit):
            ...
            red_hue_pen.pencolor(
                150, count % 255, random.randint(25, 100) + count % 100
            )
            pa.rt(my_angle)
            pa.fd(count / my_length)
            f.save_thumb()
...
def colorful_mandala_extended():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        count = 0
        turtle.bgcolor(0, 0, 0)
        count_limit = 650  # Default is 650
        my_pensize = 150  # Default is 150
...
def glorious_mandala_a():  # Based on Awesome Manadala
    ...
    for a.i in range(len(a.i_angle)):
        ...
        pensize_b = 120
        count = 0
        count_limit = 637  # 360 is default, use lower number for testing.
        ...
        for count in range(int(count_limit * 0.66)):
            pa = green_hue_pen
            pb = gold_hue_pen
            if count <= 255:
...
def glorious_mandala_b():  # Based on Awesome Manadala
    global my_project, my_angle, my_title, my_script, str_angles, count, count_limit, pa, pb
    ...
    for a.i in range(len(a.i_angle)):
        ...
        pensize_b = 50  # 120
        count = 0
        count_limit = 382  # 510 is default, adjust as needed.
        turtle.colormode(255)
        turtle.bgcolor(10, 30, 40)
        make_turtle_pa()
        make_turtle_pb()
        ...
        Lg.logger.info("Starting creation of sequential screenshots")
        Lg.logger.info(
            f"{my_script}:Starting First Loop of 4 of {t.my_title} @ {Tm.this_time}"
        )
        count = 0
        all_pens_home()
        for count in range(count_limit):
        ...
        all_pens_home()
        Lg.logger.info(
            f"{my_script}:Starting Second Loop of 4 of {t.my_title} @ {Tm.this_time}"
        )
        for count in range(count_limit):
            pa = red_hue_pen
            pb = orange_hue_pen
        ...
        # Third Pass
        Lg.logger.info(
            f"{my_script}:Starting Third Loop of 4 of {t.my_title} @ {Tm.this_time}"
        )
        all_pens_home()
        count = 0
        for count in range(count_limit):
        ...
        all_pens_home()
        Lg.logger.info(
            f"{my_script}:Starting Fourth Loop of 4 of {t.my_title} @ {Tm.this_time}"
        )
        for count in range(count_limit):
            pa = pa
            pb = dark_hue_pen
...
def random_mandala():
    ...
    for a.i in range(len(a.i_angle)):
        my_angle = a.i_angle_float[a.i]
        make_p_turtles()  # Creates turtle instances
        count_limit = 25  # 765 is default; Adjust as needed for output to fit screen; use lower or higher number for testing
        R, B, L, M = (
            0,
            random.randint(64, 127),
            random.randint(127, 255),
...
def reversing_simple_mandala():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        #         Lg.logger.info('Current angle to be drawn is ' + str(my_angle))
        count = 0
        count_limit = 1530  # default is 1785
...
def bold_mandala():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        count = 0
        #     count_limit = count
        for count in range(count_limit):  # 255 is default. Use lower number for testing.
...
def glorious_mandala():  # Based on Awesome Manadala
    ...
    for a.i in range(len(a.i_angle)):
        ...
        set_gold_pen_home()
        set_blue_pen_home()
        count_limit = 1275  # Default is 1275
        for count in range(count_limit):
...
def Fantastic_Mandala():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        count = 0
        count_limit = 1020  # default is 1020; use lower number for testing
...
def awesome_mandala_extended():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        for count in range(count_limit):  # First pass
            pick_red()  # Pen lu
            red_hue_pen.pensize(count / my_pensize)
        ...
        for count in range(count_limit):  # 3000 is default, use any number Third pass
            pick_indigo()  # Pen li
            pick_gold()  # Pen go
            indigo_hue_pen.pensize(count / my_pensize)
            indigo_hue_pen.rt(my_angle)
            ...
            if count == count_limit:
                Lg.logger.info(
                    "Completed Loop # " + str(count) + "of Third Pass @ " + Tm.this_time
                )
                f.save_final_thumb()
        all_pens_home()
        count = 0
        ...
        for count in range(count_limit):  # 3000 is default, use any number Third pass
            ...
            if count == count_limit:
                Lg.logger.info(
                    "Completed Loop # " + str(count) + "of Fifth Pass @ " + Tm.this_time
                )
                f.save_final_thumb()
        all_pens_home()
        count = 0
        ...
        for count in range(count_limit):  # 3000 is default, use any number Third pass
            ...
            if count == count_limit:
                Lg.logger.info(
                    "Completed Loop # " + str(count) + "of Sixth Pass @ " + Tm.this_time
                )
                f.save_final_thumb()
    produce_video()
    finalize()
...
def joyous_mandala():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        count_limit = 1800
        Lg.logger.info("The valu of count_limit is: " + str(count_limit))
        Lg.logger.info("Starting main script at " + str(Tm.this_time))
        for my_loops in range(1, 3):
            Lg.logger.info(
                "Starting first pass of  loop  "
                + str(my_loops)
                + "@ "
                + str(Tm.this_time)
            )
            ...
            for count in range(count_limit):
                ...
                if my_loops == 2:
                    pick_magenta()
                    gold_hue_pen.pencolor(
                        my_loops * 80, my_loops * count % 255, count_limit - count % 152
                    )
                    gold_hue_pen.pensize(count / my_pensize / my_loops)
                    ...
                    pick_green()
                    gold_hue_pen.pencolor(
                        my_loops * count % 255, my_loops * 80, count_limit - count % 127
                    )
                    gold_hue_pen.pensize(count / my_pensize / my_loops)
...
#  Group B Module_10
# +++++++++++MODULE+++++++++++++++++++++++++++++++++++++++++++++++++++++
# This module outputs 10 minute duration @ 1800 count_limit setting, and 14 minutes @ 2040 count_limit setting
...
def stupendous_mandala():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        count_limit = (
            255 * 6
        )  # Default is 2040; make it a multiple of 255 to sync end of cycle to 255 RGB modulo functions
        ...
        for count in range(count_limit):  # Second Pass
            ...
            pc.pencolor(
                count % 127, count % 255, 255 - count % 255
            )  # Pen gold_hue_pen (Pen 3)
            turtle.bgcolor(0, 10, 0)
            run_stupendous()
        pa = red_hue_pen
        pb = magenta_hue_pen
        ...
        for count in range(count_limit):  # Third Pass
            pa.pencolor(
                127 - count % 127, 255 - count % 255, count % 255
            )  # Pen red_hue_pen (Pen 1)
            pb.pencolor(
                count % 255, my_random, 255 - count % 255
            )  # Pen magenta_hue_pen  (Pen 2)
        ...
        for count in range(count_limit):  # Fourth Pass
            ...
            pb.pencolor(
                count % 255, 255 - count % 255, 255 - count % 255
            )  # Pen green_hue_pen (Pen 2)
            pa.pencolor(
                count % 127, count % 255, 255 - count % 255
            )  # Pen gold_hue_pen (Pen 3)
...
def courage_mandala():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        count = 0
        count_limit = 255  # Best with multiples of 255
        fd_01, fd_02, fd_03, fd_04 = 1.2, 1.2, 1.2, 1.2
        circ_01, circ_02, circ_03 = 18, 24, 32
        bk_01 = 6
        Lg.logger.info(f"{my_script}:First pass starting @ {Tm.this_time}")
        for count in range(
            count_limit
        ...
        reset_random_hue_pens()
        all_pens_home()
        Lg.logger.info(f"{my_script}:Second pass starting @ {Tm.this_time}")
        for count in range(
            count_limit
        ):  # 600 is default. Use lower number for testing. Loop count is  limited by maximum color value of 255 (0-255).
            turtle.bgcolor(0, 0, 0)
        ...
        reset_random_hue_pens()
        all_pens_home()
        Lg.logger.info(f"{my_script}:Third pass starting @ {Tm.this_time}")
        for count in range(
            count_limit
        ):  # 600 is default. Use lower number for testing. Loop count is  limited by maximum color value of 255 (0-255).
            turtle.bgcolor(0, 0, 0)
        ...
        reset_random_hue_pens()
        all_pens_home()
        Lg.logger.info(f"{my_script}:Fourth pass starting @ {Tm.this_time}")
        for count in range(
            count_limit
        ):  # 600 is default. Use lower number for testing. Loop count is  limited by maximum color value of 255 (0-255).
            turtle.bgcolor(0, 0, 0)
        ...
        reset_random_hue_pens()
        all_pens_home()
        Lg.logger.info(f"{my_script}:Fifth pass starting @ {Tm.this_time}")
        for count in range(
            count_limit
        ):  # 600 is default. Use lower number for testing. Loop count is  limited by maximum color value of 255 (0-255).
            turtle.bgcolor(0, 0, 0)
        ...
        reset_random_hue_pens()
        all_pens_home()
        Lg.logger.info(f"{my_script}:Sixth pass starting @ {Tm.this_time}")
        for count in range(
            count_limit
        ):  # 600 is default. Use lower number for testing. Loop count is  limited by maximum color value of 255 (0-255).
        ...
        reset_random_hue_pens()
        all_pens_home()
        Lg.logger.info(f"{my_script}:Seventh pass starting @ {Tm.this_time}")
        for count in range(
            count_limit
        ):  # 600 is default. Use lower number for testing. Loop count is  limited by maximum color value of 255 (0-255).
            turtle.bgcolor(0, 0, 0)
        ...
        reset_random_hue_pens()
        all_pens_home()
        Lg.logger.info(f"{my_script}:Eighth pass starting @ {Tm.this_time}")
        for count in range(
            count_limit
        ):  # 600 is default. Use lower number for testing. Loop count is  limited by maximum color value of 255 (0-255).
            turtle.bgcolor(0, 0, 0)  # .bg_fade_skyblue_to_dark()
        ...
        reset_random_hue_pens()
        all_pens_home()
        Lg.logger.info(f"{my_script}:Ninth pass starting @ {Tm.this_time}")
        for count in range(
            count_limit
        ):  # 600 is default. Use lower number for testing. Loop count is  limited by maximum color value of 255 (0-255).
            turtle.bgcolor(0, 0, 0)
...
def spectrum_mandala():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        count = 0
        count_limit = 509  # Best with multiples of 255
        fd_01, fd_02, fd_03, fd_04 = 1.2, 1.2, 1.2, 1.2
        circ_01, circ_02, circ_03 = 18, 24, 32
        bk_01 = 6
        ...
        Lg.logger.info(
            f"{my_script}:Third pass starting @ {Tm.this_time}"
        )  # Green Band of Spectrum
...
def Brave_mandala_extended():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        for r in range(2):
            ...
            for count in range(count_limit + 3):  # Second Pass
                pa.pensize(count / my_pensize)
                pa.left(my_angle)
                pa.fd(count / phi)
                f.save_thumb()  # Screenshot as a png
            ...
            for count in range(count_limit + 6):  # Third Pass
                pa.pensize(count / my_pensize)
                pa.left(my_angle)
                pa.fd(count / phi)
                f.save_thumb()  # Screenshot as a png
            ...
            for count in range(count_limit + 12):  # Fourth Pass
                pa.pensize(count / my_pensize)
                pa.left(my_angle)
                pa.fd(count / phi)
                f.save_thumb()  # Screenshot as a png
            ...
            for count in range(count_limit + 18):  # Fifth Pass
                pa.pensize(count / my_pensize)
                pa.left(my_angle)
                pa.fd(count / phi)
                f.save_thumb()  # Screenshot as a png
            ...
            for count in range(count_limit + 24):  # Sixth Pass
                pa.pensize(count / my_pensize)
                pa.left(my_angle)
                pa.fd(count / phi)
                f.save_thumb()  # Screenshot as a png
            ...
            for count in range(count_limit + 30):  # Seventh Pass
...
def dark_mandala():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        my_angle = math.trunc(float(a.i_angle_auto[a.i]))
        Lg.logger.info(
            f"{my_script}:DarkMandala: Current angle to be drawn is {my_angle:.2f}"
        )
...
def mighty_awesome_mandala():  # Based on pretty_awesome Mandala
    global my_project, my_angle, my_title, my_script, str_angles, count_limit, count, pa, pb
    ...
    for a.i in range(len(a.i_angle)):
        ...
        my_angle = float(a.i_angle_float[a.i])
        Lg.logger.info(
            f"{my_script}:MightyAwesomeMandala:Current angle to be drawn is {my_angle:.2f}"
        )
        t.my_title = (
            f"MightyAwesomeMandala: {my_angle:.2f} Degrees Angle and {au.my_track}"
        )
        turtle.title(t.my_title)
        turtle.bgcolor(0, 0, 10)
        all_pens_home()
        ...
        count_limit = 765  # Default is 510
        count = 0
        my_pensize = 72
        make_turtle_pa()
        make_turtle_pb()
        ...
        Lg.logger.info(
            f"{my_script}:MightyAwesomeMandala:Starting First Pass @ {Tm.this_time}"
        )
        for count in range(count_limit):
            ...
            do_mighty_awesome()
            Lg.logger.info(
                f"MightyAwesomeMandala:Completed loop count {count} of First Pass, {my_angle}: {Tm.this_time}"
        ...
        Lg.logger.info(f"{my_script}:MightyAwesomeMandala:The value of hue L is {L}")
        Lg.logger.info(
            f"{my_script}:MightyAwesomeMandala:Starting Second Pass @ {Tm.this_time}"
        )
        reset_pa_pb()
        for count in range(count_limit):  # Second pass
            ...
            do_mighty_awesome()
            Lg.logger.info(
                f"{my_script}:MightyAwesomeMandala:Completed loop count {count} of Second Pass, {my_angle}: {Tm.this_time}"
            )  # Uncomment for testing
        count = 0
        R = random.randint(10, 200)
        Lg.logger.info(f"{my_script}:MightyAwesomeMandala:The value of hue R is {R}")
        ...
        Lg.logger.info(f"{my_script}:MightyAwesomeMandala:The value of hue L is {L}")
        Lg.logger.info(
            f"{my_script}:MightyAwesomeMandala:Starting Third Pass @ {Tm.this_time}"
        )
        reset_pa_pb()
        for count in range(count_limit):  # Third pass
            ...
            do_mighty_awesome()
            Lg.logger.info(
                f"MightyAwesomeMandala:Completed loop count {count} of Third Pass, {my_angle}: {Tm.this_time}"
            )
        count = 0
        R = random.randint(10, 200)
        ...
        Lg.logger.info(f"{my_script}:MightyAwesomeMandala:The value of hue L is {L}")
        Lg.logger.info(
            f"{my_script}:MightyAwesomeMandala:Starting Fourth Pass @ {Tm.this_time}"
        )
        reset_pa_pb()
        for count in range(count_limit):  # Fourth Pass
            ...
            do_mighty_awesome()
            Lg.logger.info(
                f"{my_script}:MightyAwesomeMandala:Completed loop count {count} of Fourth Pass, {my_angle}: {Tm.this_time}"
            )
        count = 0
        R = random.randint(10, 200)
        Lg.logger.info(f"{my_script}:MightyAwesomeMandala:The value of hue R is {R}")
        ...
        for count in range(count_limit):  # Fifth pass
            ...
            do_mighty_awesome()
            Lg.logger.info(
                f"{my_script}:MightyAwesomeMandala:Completed loop count {count} of Fifth Pass, {my_angle}: {Tm.this_time}"
            )
        reset_pa_pb()
    produce_reversing_video()
    finalize()
...
def iridescent_polygram():  # Uses 2 pens with offset phi angle
    ...
    for a.i in range(len(a.i_angle)):
        ...
        while count in range(0, 255):  # 255 is default. Use lower number for testing.
            ...
            magenta_hue_pen.pencolor(
                random.randint(100, 200),
                random.randint(0, 75) + count % 150,
                random.randint(175, 255) - count % 150,
            )
            pa.pencolor(
                random.randint(0, 128) + count % 126,
                random.randint(150, 255) - count % 126,
                random.randint(100, 200),
            )
            pa.circle(count - pi, -my_angle, 8)
            magenta_hue_pen.circle(
                count / pi, my_angle * phi
...
def hued_gradiant_mandala():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        pa.left(my_angle / 2)
        pb.left(my_angle / 2)
        count_limit = 382  # Default is 382
        count = 0
        my_pensize = 48
        make_turtle_pa()
        make_turtle_pb()
...
def gradiant_mandala():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        for count in range(
            252
        ):  # 250 is default. Use lower number for testing, 300 for audio add.
            pa.pensize(count / 18)
            pa.right(my_angle)
            R = 0
            G = 255
...
def growing_yin_yang():  # Published to YouTube 11/11/2021
    ...
    for count in range(
        400
    ):  # 360 is default. Use lower number for testing. The higher the number, the longer the show.
        pa.pensize(count / 18)
        pa.color(0, 0, 0)
        pa.left(-my_angle - phi)
        pa.circle(count / 2)
...
def multi_hued_polygram():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        for count in range(
            500
        ):  # Determines size of graphic on the screen by the count of loops
            pick_green()
            pick_gold()
            R = 0
            G = 255
...
def color_shifting_mandala():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        while (
            count <= 360
        ):  # 360 is default for audio clip add, use lower number for testing
            pa.pensize(count / 90)
            pa.dot(3)
...
def gold_red_mandala():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        for count in range(
            300
        ):  # 252 is default, use lower number for testing; 300 for audio clip add
            pick_gold()
            pick_red()
            pick_light()
...
def ribbons_mandala():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        pa.pensize(1)
        pa.speed(0)
        count_limit = 25  # 765 is default, use lower number for testing
        for count in range(count_limit):
...
def cloverleaf_extended():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        my_pensize = count / 56
        global count_limit
        count_limit = 396  # 400 is default; use any number for testing, etc
        Lg.logger.info(" The program will loop  " + str(count_limit) + "  times")
        for i in range(2):
            t.ld.penup()
            t.ld.setpos(0, 0)
            t.ld.seth(my_angle)
            ...
            for count in range(count_limit):
                ...
                if i == 0:
                    t.ld.pencolor(
                        255 - count % 255, 255 - count % 255, count % 255
                    )  # Fade yellwo to blue
                else:
                ...
                Lg.logger.info(
                    "t.ld.pensize is  " + str(my_pensize) + "( for  " + str(count)
                )
                f.save_thumb()
        ...
        for i in range(2):
            ...
            for count in range(count_limit):
                ...
                if i == 0:
                    t.lc.pencolor(
                        count % 25, 255 - count % 250, 255 - count % 100
                    )  # Fade yellow to red
                else:
...
def majestic_mandala_extended():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        for r in range(3):
            ...
            Lg.logger.info(
                f"{my_script}:The soundtrack selected for this project is {au.my_track}"
            )
...
def wall_show():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        Lg.logger.info(
            f"{my_script}:The soundtrack being used for this show is: {au.my_track}"
        )
        Lg.logger.info(f"{my_script}:The featured angle is {my_angle}")
...
def black_seed():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        my_base = int(
            3600 / my_angle
        )  # for loop integrity of the number of loops of the ce pen
        pa.speed(30)
        pa.pensize(36)
        random_hue_pen.speed(30)
        random_hue_pen.pensize(54)
...
def dark_mandala_extended():  # Work on some more
    ...
    Lg.logger.info(
        f"{my_script}:Status: Creation Date: 12/2/2023; Last Update: {Tm.my_date}"
    )
    Lg.logger.info(
        f"{my_script}:Uses local randomized hues pens magenta, indigo, green, red, gold, dark, light."
    )
    Lg.logger.info(
        f"{my_script}:Phi constant is used to apply increasing line size with each loop."
    )
    Lg.logger.info(
        f"{my_script}:Includes special center black dot pen. Duration is based upon quantity of angles selected."
    )
    startup_script()
    make_folder()
    (
    ...
    for a.i in range(len(a.i_angle)):
        ...
        Lg.logger.info(f"{my_script}:Starting Loop One @ {Tm.this_time}")
        for count in range(count_limit):
            Q_seed.dot(3)
        ...
        Lg.logger.info(f"{my_script}:Starting Loop Two @ {Tm.this_time}")
        count = 0
        for count in range(count_limit):
            Q_seed.dot(3)
        ...
        #         reset_all_pens()
        Lg.logger.info(f"{my_script}:Starting Loop Three @ {Tm.this_time}")
        count = 0
        for count in range(count_limit):
            Q_seed.dot(3)
...
green_hue_pen, yellow_hue_pen,
magenta_hue_pen, light_hue_pen,
indigo_hue_pen, gold_hue_pen,
red_hue_pen, blue_hue_pen,
orange_hue_pen, random_hue_pen,
green_hue_pen, cyan_hue_pen.
Last run Date> 2/16/2024
Estimated 5.1 minutes per angle completed.
'''
def Plain__one():
    global my_project, my_angle, my_title, my_script, str_angles, count, count_limit
    my_script = "Plain Mandala One"
    Lg.logger.info(f" Plain Mandala_one run date: {Tm.my_date}")
    ...
    for a.i in range(len(a.i_angle)):
        ...
        for count in range(count_limit):
            ...
            pick_yellow()  # Pen yellow_hue_pen
            for foo in range(2):
                yellow_hue_pen.left(my_angle/2)
                yellow_hue_pen.fd(count/my_distance *2)
                f.save_thumb()
                green_hue_pen.rt(my_angle)
...
def Plain__two():
    ...
    for a.i in range(len(a.i_angle)):
        ...
        for count in range(count_limit):
            ...
            pick_light()  # Pen light_hue_pen
            for foo in range(2):
                light_hue_pen.left(my_angle/2)
                light_hue_pen.fd(count/my_distance *2)
                f.save_thumb()
                magenta_hue_pen.rt(my_angle)
                magenta_hue_pen.fd(count/my_distance)
                process_thumbs()
                pa.pensize(count / my_pensize)
                magenta_hue_pen.pensize(count / my_pensize)
...
def Plain__three():
    global my_project, my_angle, my_title, my_script, str_angles, count, count_limit
    Lg.logger.info(f" indigo_hue_pen, gold_hue_pen")
    ...
    for a.i in range(len(a.i_angle)):
        ...
        for count in range(count_limit):
            ...
            for foo in range(2):
                ...
                indigo_hue_pen.fd(count/my_distance *2)
                f.save_thumb()
                gold_hue_pen.rt(my_angle)
                gold_hue_pen.fd(count/my_distance)
                process_thumbs()
                indigo_hue_pen.pensize(count / my_pensize)
                gold_hue_pen.pensize(count / my_pensize)
        clear_indigo_hue_pen()
...
def Plain__four():
    global my_project, my_angle, my_title, my_script, str_angles, count, count_limit
    Lg.logger.info(f" red_hue_pen, blue_hue_pen")
    ...
    for a.i in range(len(a.i_angle)):
        ...
        for count in range(count_limit):
            ...
            for foo in range(2):
                red_hue_pen.left(my_angle/2)
                red_hue_pen.fd(count/my_distance *2)
                f.save_thumb()
                blue_hue_pen.rt(my_angle)
                blue_hue_pen.fd(count/my_distance)
                process_thumbs()
                red_hue_pen.pensize(count / my_pensize)
                blue_hue_pen.pensize(count / my_pensize)
        clear_red_hue_pen()
...
def Plain__five():
    global my_project, my_angle, my_title, my_script, str_angles, count, count_limit
    Lg.logger.info(f" orange_hue_pen, random_hue_pen")
    ...
    for a.i in range(len(a.i_angle)):
        ...
        for count in range(count_limit):
            ...
            for foo in range(2):
                ...
                orange_hue_pen.fd(count/my_distance *2)
                f.save_thumb()
                random_hue_pen.rt(my_angle)
                random_hue_pen.fd(count/my_distance)
                process_thumbs()
                orange_hue_pen.pensize(count / my_pensize)
                random_hue_pen.pensize(count / my_pensize)
        clear_orange_hue_pen()
        clear_random_hue_pen()
    produce_reversing_video()
    finalize()    
# Plain__five()
# exit()       
...
def Plain__six():
    global my_project, my_angle, my_title, my_script, str_angles, count, count_limit
    Lg.logger.info(f" green_hue_pen, cyan_hue_pen.")
    ...
    for a.i in range(len(a.i_angle)):
        ...
        for count in range(count_limit):
            turtle.bgcolor(0, 10, 0)
            pick_green()  # Pen green_hue_pen
            pick_cyan()  # Pen cyan_hue_pen
            for foo in range(2):
                cyan_hue_pen.left(my_angle/2)
                cyan_hue_pen.fd(count/my_distance *2)
                f.save_thumb()
                green_hue_pen.rt(my_angle)
                green_hue_pen.fd(count/my_distance)
                process_thumbs()
...
def Durable_mandala():
    ...
    for a.i  in range( len(a.i_angle_float)):
        ...
        red_hue_pen.pensize(1)
        green_hue_pen.pensize(1)
        cyan_hue_pen.pensize(3)
        for count in range(count_limit): # 255 is default. Use lower number for testing
            pick_red()
            pick_green()
            pick_cyan()
            cyan_hue_pen.dot(count / 9)
            red_hue_pen.pensize(count / 110)
            red_hue_pen.left(-my_angle)
            red_hue_pen.fd(count * phi)
            f.save_thumb()
            red_hue_pen.left(-my_angle)
            red_hue_pen.fd(count / 12)
            f.save_thumb()
            green_hue_pen.pensize(count / 110)
            green_hue_pen.left(my_angle / 2)
            green_hue_pen.fd(count * phi)
            f.save_thumb()
            green_hue_pen.left(-my_angle)
            green_hue_pen.fd(count / 12)
            f.save_thumb()
            cyan_hue_pen.pensize(count / 110)
            cyan_hue_pen.left(my_angle)
            cyan_hue_pen.dot(count / phi / 18)
            cyan_hue_pen.fd(count + pi)
            f.save_thumb()
            green_hue_pen.left(my_angle)
            green_hue_pen.circle(count /9, my_angle, 9)
            process_thumbs()
...
def Bounteous_One():
    Lg.logger.info("##########################################################")
    global my_project, my_angle, my_title, my_script, str_angles, count_limit, count, pa, pb, my_pensize
    my_script = "Bounteous One"
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project}"
    Lg.logger.info(f'Bounteous One employs pens pa, pb, and blue_hue_pen.')
    for a.i in range(len(a.i_angle)):
        my_angle = a.i_angle_float[a.i]
        Tm.set_time()
        Lg.logger.info(
...
def Bounteous_Two():
    Lg.logger.info("##########################################################")
    global my_project, my_angle, my_title, my_script, str_angles, count_limit, count, pa, pb, my_pensize
    my_script = "Bounteous Two"
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project}"
    Lg.logger.info(f'Bounteous Two employs pens pa, pb, and magenta_hue_pen.')
    for a.i in range(len(a.i_angle)):
        my_angle = a.i_angle_float[a.i]
        Tm.set_time()
        Lg.logger.info(
...
def Bounteous_Three():
    Lg.logger.info("##########################################################")
    global my_project, my_angle, my_title, my_script, str_angles, count_limit, count, pa, pb, my_pensize
    my_script = "Bounteous Three"
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project}"
    Lg.logger.info(f'Bounteous Three employs pens pa, pb, and green_hue_pen.')
    for a.i in range(len(a.i_angle)):
        my_angle = a.i_angle_float[a.i]
        Tm.set_time()
        Lg.logger.info(
...
def Bounteous_Four():
    Lg.logger.info("##########################################################")
    global my_project, my_angle, my_title, my_script, str_angles, count_limit, count, pa, pb, my_pensize
    my_script = "Bounteous Four"
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project}"
    Lg.logger.info(f'Bounteous Four employs pens pa, pb, and gold_hue_pen.')
    for a.i in range(len(a.i_angle)):
        my_angle = a.i_angle_float[a.i]
        Tm.set_time()
        Lg.logger.info(
...
def Bounteous_Five():
    Lg.logger.info("##########################################################")
    global my_project, my_angle, my_title, my_script, str_angles, count_limit, count, pa, pb, my_pensize
    my_script = "Bounteous Five"
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project}"
    Lg.logger.info(f'Bounteous Five employs pens pa, pb, and indigo_hue_pen.')
    for a.i in range(len(a.i_angle)):
        my_angle = a.i_angle_float[a.i]
        Tm.set_time()
        Lg.logger.info(
...
def Bounteous_Six():
    Lg.logger.info("##########################################################")
    global my_project, my_angle, my_title, my_script, str_angles, count_limit, count, pa, pb, my_pensize
    my_script = "Bounteous Six"
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project} "
    Lg.logger.info(f'Bounteous Six employs pens pa, pb, and orange_hue_pen.')
    for a.i in range(len(a.i_angle)):
        my_angle = a.i_angle_float[a.i]
        Tm.set_time()
        Lg.logger.info(
...
def Bounteous_Seven():
    Lg.logger.info("##########################################################")
    global my_project, my_angle, my_title, my_script, str_angles, count_limit, count, pa, pb, my_pensize
    my_script = "Bounteous Seven"
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project}"
    Lg.logger.info(f'Bounteous Seven employs pens pa, pb, and yellow_hue_pen.')
    for a.i in range(len(a.i_angle)):
        my_angle = a.i_angle_float[a.i]
        Tm.set_time()
        Lg.logger.info(
...
def Bounteous_Eight():
    Lg.logger.info("##########################################################")
    global my_project, my_angle, my_title, my_script, str_angles, count_limit, count, pa, pb, my_pensize
    my_script = "Bounteous Eight"
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project}"
    Lg.logger.info(f'Bounteous Eight employs pens pa, pb, and light_hue_pen.')
    for a.i in range(len(a.i_angle)):
        my_angle = a.i_angle_float[a.i]
        Tm.set_time()
        Lg.logger.info(
...
# +++++++++++MODULE+++++++++++++++++++++++++++++++++++++++++++++++++++++
'''Estimated .16 minutes per angle completed
'''
def Gradiant_one():
    global my_project, my_angle, my_title, my_script, str_angles,count, count_limit
    my_script = "Gradiant Mandala One"
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project}"
    for a.i in range(len(a.i_angle)):
        my_angle = a.i_angle_float[a.i]
        Lg.logger.info(f"The offset angle value is {my_angle * phi}")
        Tm.set_time()
        Lg.logger.info(
            f"{my_script}:Current angle to be drawn is {my_angle} @ {Tm.this_time}"
        )
        ...
        for count in range(
            ...
            pa.fd(count * .25)
            f.save_thumb()
            magenta_hue_pen.circle(count * .25, my_angle, 3)
            magenta_hue_pen.pensize(count / 200)
            process_thumbs()
        clear_pa_pen()
        clear_magenta_hue_pen()
    produce_reversing_video()
...
def Gradiant_two():
    global my_project, my_angle, my_title, my_script, str_angles,count, count_limit
    my_script = "Gradiant Mandala Two"
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project}"
    for a.i in range(len(a.i_angle)):
        ...
        for count in range(
            ...
            G = 255
            B = 255
            pa.color(B - count % 255, G - count % 255, R + count % 255)
            pa.fd(count * .25)
            f.save_thumb()
            gold_hue_pen.circle(count * .25, my_angle, 3)
            gold_hue_pen.pensize(count / 200)
            process_thumbs()
        clear_pa_pen()
        clear_gold_hue_pen()
    produce_reversing_video()
...
def Gradiant_three():
    global my_project, my_angle, my_title, my_script, str_angles,count, count_limit
    my_script = "Gradiant Mandala Three"
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project}"
    for a.i in range(len(a.i_angle)):
        ...
        for count in range(
            ...
            G = 255
            B = 255
            pa.color(B - count % 150, R + count % 255, G - count % 255)
            pa.fd(count * .25)
            f.save_thumb()
            green_hue_pen.circle(count * .25, my_angle, 3)
            green_hue_pen.pensize(count / 200)
            process_thumbs()
        clear_pa_pen()
        clear_green_hue_pen()
    produce_reversing_video()
...
def Gradiant_four():
    global my_project, my_angle, my_title, my_script, str_angles,count, count_limit
    my_script = "Gradiant Mandala Four"
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project}"
    for a.i in range(len(a.i_angle)):
        ...
        for count in range(
            ...
            G = 255
            B = 255
            pa.color(B - count % 150, G - count % 255, R + count % 150)
            pa.fd(count * .25)
            f.save_thumb()
            blue_hue_pen.circle(count * .25, my_angle, 3)
            blue_hue_pen.pensize(count / 200)
            process_thumbs()
        clear_pa_pen()
        clear_blue_hue_pen()
    produce_reversing_video()
...
def Gradiant_five():
    global my_project, my_angle, my_title, my_script, str_angles,count, count_limit
    my_script = "Gradiant Mandala Five"
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project}"
    for a.i in range(len(a.i_angle)):
        ...
        for count in range(count_limit):
            ...
            G = 255
            B = 255
            pa.color(R + count % 255,  B - count % 150, G - count % 50, )
            pa.fd(count * .25)
            f.save_thumb()
            red_hue_pen.circle(count * .25, my_angle, 3)
            red_hue_pen.pensize(count / 200)
            process_thumbs()
        clear_pa_pen()
        clear_red_hue_pen()
    produce_reversing_video()
...
# +++++++++++MODULE Bold Mandala Group+++++++++++++++++++++++++++++++++++++++++++++++++++++

def Bold__one():
    global  my_angle, my_title, my_script, str_angles, count, count_limit
    my_script = "Bold Mandala One"
    Lg.logger.info(f"{my_script}: Employs random_hued pens gold, indigo")
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project}"
    for a.i in range(len(a.i_angle_float)):
        my_angle = a.i_angle_float[a.i]
        Lg.logger.info(f"Current angle to be drawn is {my_angle}")
        turtle.title(f"{my_script}: {my_angle} angle ")
        my_pensize = 30
        turtle.bgcolor(0, 255, 255)
        count_limit = 510 # 255 is default. Use lower number for testing.
        count = 0
        for count in range(count_limit):  
            pick_indigo()  # indigo_hue_pen;Indigo hues
            pick_gold()  # gold_hue_pen; Gold hues
            ...
            indigo_hue_pen.setpos(0, 0)
            indigo_hue_pen.pendown()
            indigo_hue_pen.pensize(count / my_pensize / 2)
            indigo_hue_pen.circle(count / 2, my_angle)
            f.save_thumb()
            gold_hue_pen.pensize(count / my_pensize)  
            gold_hue_pen.left(my_angle)
            gold_hue_pen.backward(count * phi)
            ...
            f.save_thumb()
            if count <= 254:
                turtle.bgcolor(0, 255 - count % 255, 255 - count % 255)
                pa.rt(my_angle / 2)
                pa.pencolor(0, 255 - count % 255, 255 - count % 255)
                pa.pensize(count / my_pensize /2)
                pa.fd(count / 5)
                f.save_thumb()
            else:
                turtle.bgcolor(0, count % 255, count % 255)
                pa.rt(my_angle / 2)
                pa.pencolor(255, random.randint(50, 150), random.randint(0, 100))
                pa.fd(count / 5)
                f.save_thumb()  
            process_thumbs()
        count = 0
...
def Bold__two():
    global  my_angle, my_title, my_script, str_angles, count, count_limit
    my_script = "Bold Mandala Two"
    Lg.logger.info(f"{my_script}: Employs random_hued pens red, blue")
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project}"
    for a.i in range(len(a.i_angle_float)):
        ...
        for count in range(count_limit):
            ...
            red_hue_pen.setpos(0, 0)
            red_hue_pen.pendown()
            red_hue_pen.pensize(count / my_pensize /2)
            red_hue_pen.circle(count /2, my_angle)  
            f.save_thumb()
            blue_hue_pen.pensize(count / my_pensize)  
            blue_hue_pen.left(my_angle)
            blue_hue_pen.backward(count * phi)
            ...
            if count <= 254:
                ...
                turtle.bgcolor(0, count % 255, count % 255)
                pa.rt(my_angle / 2)
                pa.pencolor(255, random.randint(50, 150), 0)
                pa.fd(count / 5)
                f.save_thumb()  
            process_thumbs()
        clear_blue_hue_pen()
...
def Bold__three():
    global  my_angle, my_title, my_script, str_angles, count, count_limit
    my_script = "Bold Mandala Three"
    Lg.logger.info(f"{my_script}: Employs random_hued pens green, yellow")
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project}"
    for a.i in range(len(a.i_angle_float)):
        ...
        for count in range(count_limit):
            ...
            green_hue_pen.setpos(0, 0)
            green_hue_pen.pendown()
            green_hue_pen.pensize(count / my_pensize / 2)
            green_hue_pen.circle(count / 2, my_angle)  
            f.save_thumb()
            yellow_hue_pen.pensize(count / my_pensize)
            yellow_hue_pen.left(my_angle)
            yellow_hue_pen.backward(count * phi)  
            f.save_thumb()
            yellow_hue_pen.left(my_angle)
            yellow_hue_pen.fd(count / 4)
            f.save_thumb()
            if count <= 254:
                turtle.bgcolor(0, 255 - count % 255, 255 - count % 255)
                pa.rt(my_angle / 2)
                ...
                turtle.bgcolor(0, count % 255, count % 255)
                pa.rt(my_angle / 2)
                pa.pencolor(0, 255, random.randint(50, 150))
                pa.fd(count / 5)
                f.save_thumb()  
            process_thumbs()
        clear_green_hue_pen()
...
def Bold__four():
    global  my_angle, my_title, my_script, str_angles, count, count_limit
    my_script = "Bold Mandala Four"
    Lg.logger.info(f"{my_script}: Employs random_hued pens magenta, random_hue")
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project}"
    for a.i in range(len(a.i_angle_float)):
        ...
        for count in range(count_limit):  # 255 is default. Use lower number for testing.
            ...
            random_hue_pen.setpos(0, 0)
            random_hue_pen.pendown()
            random_hue_pen.pensize(count / my_pensize /2)
            random_hue_pen.circle(count / 2, my_angle)
            f.save_thumb()
            magenta_hue_pen.pensize(count / my_pensize)
            magenta_hue_pen.left(my_angle)
            magenta_hue_pen.backward(count * phi)  # Magenta pen
            ...
            if count <= 254:
                ...
                turtle.bgcolor(0, count % 255, count % 255)
                pa.rt(my_angle / 2)
                pa.pencolor(random.randint(50, 150), 255, 0)
                pa.fd(count / 5)
                f.save_thumb()  
            process_thumbs()
        clear_random_hue_pen()
...
def Bold__five():
    global  my_angle, my_title, my_script, str_angles, count, count_limit
    my_script = "Bold Mandala Five"
    Lg.logger.info(f"{my_script}: Employs random_hued pens orange, cyan")
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project}"
    for a.i in range(len(a.i_angle_float)):
        my_angle = a.i_angle_float[a.i]
        Lg.logger.info(f"Current angle to be drawn is {my_angle}")
        turtle.title(f"Bold Mandala Five: {my_angle} angle ")
        my_pensize = 30
        turtle.bgcolor(0, 255, 255)
        count_limit = 510 # 255 is default. Use lower number for testing.
        count = 0        
        for count in range(count_limit):
            ...
            orange_hue_pen.setpos(0, 0)
            orange_hue_pen.pendown()
            orange_hue_pen.pensize(count / my_pensize /2)
            orange_hue_pen.circle(count / 2, my_angle)  
            f.save_thumb()
            cyan_hue_pen.pensize(count / my_pensize)
            cyan_hue_pen.left(my_angle)
            cyan_hue_pen.backward(count * phi)  
            f.save_thumb()
            cyan_hue_pen.left(my_angle)
            ...
            if count <= 254:
                ...
                turtle.bgcolor(0, count % 255, count % 255)
                pa.rt(my_angle / 2)
                pa.pencolor(0, random.randint(50, 150), 255)
                pa.fd(count / 5)
                f.save_thumb()  
            process_thumbs()
        clear_orange_hue_pen()
...
def Bold__six():
    global  my_angle, my_title, my_script, str_angles, count, count_limit
    my_script = "Bold Mandala Six"
    Lg.logger.info(f"{my_script}: Employs random_hued pens dark, light")
    startup_script()
    make_folder()
    t.my_angles, t.my_splash = str_angles, f"{my_project}"
    for a.i in range(len(a.i_angle_float)):
        my_angle = a.i_angle_float[a.i]
        Lg.logger.info(f"Current angle to be drawn is {my_angle}")
        turtle.title(f"Bold Mandala Six: {my_angle} angle")
        my_pensize = 30
        count_limit = 510 # 255 is default. Use lower number for testing.
        count = 0
        pa.penup()
        ...
        for count in range(count_limit):
            pick_dark()  # dark_hue_pen; dark hues
            pick_light()  #  light hue_pen; light hues
            Q_seed.dot(4)
            f.save_thumb()
            dark_hue_pen.left(my_angle)
            dark_hue_pen.penup()
            dark_hue_pen.setpos(0, 0)
            dark_hue_pen.pendown()
            dark_hue_pen.pensize(count / my_pensize / 2)
            dark_hue_pen.circle(count / 2, my_angle)
            f.save_thumb()
            light_hue_pen.pensize(count / my_pensize)
            light_hue_pen.left(my_angle)
            light_hue_pen.backward(count * phi)  # light pen
            f.save_thumb()
            light_hue_pen.left(my_angle)
            light_hue_pen.fd(count / 4)
            f.save_thumb()
            if count <= 254:
                turtle.bgcolor(0, 255 - count % 255, 255 - count % 255)
                pa.rt(my_angle / 2)
...
def Resonance_One():
    global my_project, my_angle, my_title, my_script, str_angles, count, count_limit, pa
    my_script = "Resonance One Mandala"
    Lg.logger.info(
        f"{my_script}:Status: Creation Date: 12/2/2023; Last Update/Run: {Tm.my_date}"
    )
    Lg.logger.info(
        f"{my_script}:Uses local turtle pens pa and orange_hue_pen; spun off from Intense Mandala."
    )
    Lg.logger.info(
    ...
    for a.i in range(len(a.i_angle)):
        ...
        f.make_reverse_png_folder()
        count_limit = 15   # Default is 2550; use lesser for testing
        R, G = 0, 0
        M, N, X, Y, Z = 255, 110, 10, 155, 255
        B = random.randint(10, 50)
        L = random.randint(150, 255)
        pick_cyan()  # cyan_hue_pen
        cyan_hue_pen.left(t.my_angle / 2)
        ...
        for count in range(count_limit):
            ...
            f.save_thumb()
            pick_cyan()  # cyan_hue_pen
            cyan_hue_pen.pensize(count / 1200)
            cyan_hue_pen.left(t.my_angle / 2)
            cyan_hue_pen.fd(count / 3)
            f.save_thumb()
            Q_seed.dot(5)
        process_thumbs()
        clear_pa_pen()
...
def Resonance_Two():
    global my_project, my_angle, my_title, my_script, str_angles, count, count_limit, pa
    my_script = "Resonance Two Mandala"
    Lg.logger.info(
        f"{my_script}:Status: Creation Date: 12/2/2023; Last Update/Run: {Tm.my_date}"
    )
    Lg.logger.info(
        f"{my_script}:Uses local turtle pens pa and indigo_hue_pen; spun off from Intense Mandala."
    )
    Lg.logger.info(
        f"{my_script}:Features first attempt to freeze the size of the mandala,"
    )
    ...
    for a.i in range(len(a.i_angle)):
        ...
        for count in range(count_limit):
            ...
            if count <= count_limit * 0.66:
                ...
                # Orange_hue_pen as light_hue_pen
                Q_seed.dot(5)
                f.save_thumb()
                orange_hue_pen.pencolor(
                    random.randrange(190, 215, 1),
                    random.randrange(191, 254, 1),
                    random.randrange(200, 253, 1),
                )
                orange_hue_pen.pensize(count / 120)
                orange_hue_pen.left(my_angle / 2)
                orange_hue_pen.fd(count / 3)
...
def benchmark_mandala():
    ...
    Lg.logger.info(
        f"{my_script}:Status: Creation Date: 12/7/2023; Last Update: {Tm.my_date}"
    )
    startup_script()
    make_folder()
    (
