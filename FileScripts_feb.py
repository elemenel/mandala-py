...
# Global declarations
global no_audio_vids, my_video_path_av, my_thumbs_path, clip_path, full_vid_path, Lg, my_angle
...
if sys.platform.startswith("linux"):
    ...
    my_mandala_pics_path = "/media/sels/Ex-01-Linux/Videos/Pictures/Mandala Final Thumbs/"
    my_thumbs = "/home/sels/Thumbs/"
    my_thumbs_library = "/media/sels/Ex-01-Linux/Thumbs_Library/"
    mandala_video_backup = "/media/sels/Ex-01-Linux/Mandala Video Library/"
    mandala_final_thumbs_library  = "/media/sels/Ex-01-Linux/MandalaPics Library/"
    final_thumbs = "/home/sels/Pictures/Final_Thumbs/"
    my_shared_drive = "/home/sels/sambashare/"
    my_backup_drive = "/media/Ex_02_Linux/MandalaBackup/"
    my_audio_backup_folder = (
        "/media/sels/Ex-01-Linux/AudioClipsPython/"
    )
    concatenate_candidates = "/home/sels/Videos/concatenate_candidates/"
    do_concatenate_folder = "/home/sels/Videos/do_concatenate/"
    concatenated_folder = "/home/sels/Videos/concatenated/"
    mandala_maker_backup = "/media/Ex_02_Linux/MandalaBackup/Full_Vids/"
    YouTube_uploads = "/home/sels/Videos/YouTube Uploads/"
#     full_vids_library = '/media/sels/Full_Vids/MandalaLibary/'
else:
    my_work_dir = "D:/"
    no_audio_vids = "D:/"
    ...
    mandala_final_thumbs_library  = "Z:/Mandala Pics Library/"
    my_shared_drive = "Y:/sambashare/"
    YouTube_uploads = "D:/YouTube uploads/"


def create_mandala_folders():
    os.makedirs(my_home_dir, exist_ok=True)
    os.makedirs(my_work_dir, exist_ok=True)
    os.makedirs(my_log_dir, exist_ok=True)
    os.makedirs(my_video_path, exist_ok=True)
    os.makedirs(no_audio_vids, exist_ok=True)
    os.makedirs(my_full_vids, exist_ok=True)
    os.makedirs(my_audio_path, exist_ok=True)
    os.makedirs(my_git_path, exist_ok=True)
    os.makedirs(my_mandala_pics_path, exist_ok=True)
    os.makedirs(mandala_final_thumbs_library, exist_ok=True)
    os.makedirs(my_thumbs, exist_ok=True)
    os.makedirs(my_thumbs_library, exist_ok = True)
    os.makedirs(final_thumbs, exist_ok=True)
    os.makedirs(my_shared_drive, exist_ok=True)
    os.makedirs(my_backup_drive, exist_ok=True)
    os.makedirs(my_audio_backup_folder, exist_ok=True)
    os.makedirs(concatenate_candidates, exist_ok=True)
    os.makedirs(do_concatenate_folder, exist_ok=True)
    os.makedirs(concatenated_folder, exist_ok= True)
    os.makedirs(mandala_maker_backup, exist_ok=True)
    os.makedirs(YouTube_uploads, exist_ok=True)
    os.makedirs( mandala_video_backup, exist_ok=True)
    Lg.logger.info(f'FileScripts:Directories necessary to the functionality of Mandala Maker have been confirmed or created')

create_mandala_folders()

# Change File Permissions to read/write
def change_file_mode():
    import subprocess

    my_directory = "/media/Ex-02-Linux/"
    subprocess.call[("chown", "sels", "my_directory")]
    subprocess.call(
        ["chmod", "-R", "+w", my_directory]
...
def make_reverse_png_folder():
    ...
    Lg.logger.info(
        f"FileScripts:New folder:  {reverse_png_folder} has been created @ {Tm.my_date_time}"
    )
    Tm.set_time()
    Lg.logger.info(
        f"FileScripts:{reverse_png_folder} has been emptied. The folder is now ready for new content @ {Tm.my_date_time}."
    )
...
def make_png_folder():
    ...
    Tm.set_time()
    Lg.logger.info(
        f"FileScripts:New folder: {png_folder} has been created @ {Tm.my_date_time} "
    )
    Lg.logger.info(
        f"FileScripts:{png_folder} has been emptied. The folder is now ready for new content @ {Tm.my_date_time}."
    )
...
def make_archive_folder():
    ...
    [
        shutil.copy(os.path.join(origin, i), os.path.join(destination, i))
...
"""-----------------------------------Screenshot Functions Section Below--------------------------------------------------------------
"""
...
#     image.save(f"{my_mandala_pics_path}{t.folder_name}-{Tm.iterable_time}.png")
    image.save(f"{final_thumbs}{t.folder_name}-{Tm.iterable_time}_final.jpeg")
    image.save(f"{mandala_final_thumbs_library}{t.folder_name}-{Tm.iterable_time}_final.jpeg")
    image.save(f"{final_thumbs}test.jpeg")


"""-----------------------------------Screenshot Functions Section Above-------------------------------------------------------------"""
...
def set_reverse_vid_env():
    Tm.set_time()
    Lg.logger.info(f"FileScripts:Starting set_reverse_vid_env() @ {Tm.my_date_time}")
    current_path()
...
def set_vid_env():
    Lg.logger.info(f"FileScripts:Starting set_vid_env() @ {Tm.my_date_time}")
    current_path()
    os.chdir(f"{my_thumbs}{t.folder_name}")  # (my_thumbs + t.folder_name)
    Lg.logger.info(
        f"FileScripts:The current folder is: {my_thumbs}{t.folder_name}"
    )  # + my_thumbs + t.folder_name)
    make_video()
    os.chdir(my_work_dir)
...
def make_video():
    ...
    Lg.logger.info(
        f"FileScripts:The directory to be sorted is {my_thumbs}{t.folder_name}"
    )
    ...
    sorted_files = sorted(
        os.listdir("."), key=os.path.getctime
    )  # Collect and sort .png files
    Lg.logger.info(f"FileScripts:Sorting process has completed @ {Tm.this_time}")
    Lg.logger.info(
        f"FileScripts:Starting video generation from sorted screenshot files @ {Tm.this_time}"
    )
    ...
    Lg.logger.info(
        f"FileScripts:Total Files to be processed into a clip: {total_png_count}"
    )
    clip_estimated_run_time = (
        total_png_count / 100
    ) * 2.90  # From most recent benchmark; actual time can vary
    Lg.logger.info(
        f"FileScripts:Estimated time to process the pngs into a video is {clip_estimated_run_time:.2f} seconds, or {clip_estimated_run_time/60:.2f} minutes"
    )
    ...
    Lg.logger.info(
        f"FileScripts: Estimated Video Duration: {estimated_video_clip_duration:.2f} seconds, or {estimated_video_clip_duration/60:.2f} minutes"
    )
    my_clip = moviepy.video.io.ImageSequenceClip.ImageSequenceClip(
        sorted_files, fps=fps
    )
    Lg.logger.info(
        f"FilesScripts:Total time to sort the image files is {elapsed_sort_time/60.:2f} minutes "
    )
    # Write sequenced png files to a single .mp4 file normally less than 10 mb, quality as good as much larger .avi file
    Lg.logger.info(f"FileScripts:Starting to write video file @ {Tm.my_date_time}")
    ...
    my_clip.write_videofile(
        f"{no_audio_vids}{t.folder_name}.mp4",
        fps=fps,
        codec="libx264",
        bitrate=None,
        audio=False,
        ...
        audio_bitrate=None,
        audio_bufsize=8000,
        temp_audiofile=my_work_dir + "/temp",
        remove_temp=True,
        write_logfile=True,
        threads=4,
        ffmpeg_params=None,
    ...
    print(
        f"FileScripts:Total time to create video from sorted files is {clip_creation_time:.2f} seconds"
    )
    Lg.logger.info(
        f"FileScripts:Total time to create video from sorted files is {clip_creation_time:.2f} seconds, or {clip_creation_time /60:.2f} minutes "
    )
    ...
    Lg.logger.info(
        f"FileScripts:mp4 no_audio duration: {my_clip.duration / 60.:2f} minutes"
    )
    # Calculate elapsed time
    make_video_end_time = timer()
    make_video_elapsed_time = make_video_end_time - make_video_start_time
    Lg.logger.info(
        f"FileScripts:Total time to run the make_video is {make_video_elapsed_time/3600.:2f} hours"
    )
    Lg.logger.info(
        f"FileScripts:Screenshot images conversion to audio-less mp4 video file completed @ {Tm.this_time}"
    )
...
def make_reversed_video():
    ...
    my_clip = moviepy.video.io.ImageSequenceClip.ImageSequenceClip(
        sorted_files + reverse_sorted_files, fps=fps
    )
    Lg.logger.info(
        f"FilesScripts:Total time to sort the image files is {elapsed_sort_time/60.:2f} minutes "
    )
    ...
    my_clip.write_videofile(
     ...
     audio_bitrate=None,
     audio_bufsize=None,
     temp_audiofile=None, # f"{my_work_dir}/temp",
     remove_temp=True,
     write_logfile=True,
     threads=8,
     ffmpeg_params=None,
    ...
    my_vidClip = f"{no_audio_vids}{t.folder_name}.mp4"
    copy_videos(origin=f"{no_audio_vids}{t.folder_name}.mp4",destination= f"{concatenate_candidates}")
    end_clip_creation_start_time = timer()
    clip_creation_time = end_clip_creation_start_time - begin_clip_creation_start_time
    print(
        f"FileScripts:Total time to create video from sorted files is {clip_creation_time:.2f} seconds"
    ...
    make_reversed_video_elapsed_time = (
        make_reversed_video_end_time - make_reversed_video_start_time
    )
    Lg.logger.info(
        f"FileScripts:Total time to run the make_reversed_video is {make_reversed_video_elapsed_time/3600.:2f} hours"
    )
    Tm.set_time()
    Lg.logger.info(
        f"FileScripts:Screenshot images conversion to audio-less mp4 video file completed @ {Tm.this_time}"
...
def merge_vids():
    ...
    os.chdir(f"{concatenated_folder}")
    video_paths  = [f for f in glob.glob(f"{do_concatenate_folder}*.mp4")]
    ...
    final_clip.write_videofile(f"{concatenated_folder}merged_video.mp4")

    print("Videos merged successfully!")
    Lg.logger.info("merge_vids:Videos merged successfully!")
...
def add_av():
    Tm.set_time()
    Lg.logger.info(f"FileScripts:Starting add_av() @ {Tm.this_time}")
    Lg.logger.info(f"FileScripts:Merging Video with Audio...")
    t.my_angle = 1224
    au.my_audio_clip ="/home/sels/Music/Audio Clips for Python/All_clips/528, 1584, 176, 52.8 Hz Steady-60min.mp3"
    video_file_name = "/home/sels/Videos/concatenated/merged_video.mp4"
    audioclip = AudioFileClip(au.my_audio_clip)
    composite_video_file_name = f"{my_full_vids}Seven Various Mandalas featuring {t.my_angle} Degrees Angles with S528, 1584, 176, 52.8 Hz Steady-60min.mp4"
#     av_start_time = timer()
    
    os.chdir(my_video_path)
    new_video = "__temp__.mp4"
    Lg.logger.info(f"FileScripts:Folder name is {video_file_name}")
    
#     os.makedirs(f"/home/sels/Videos/do_concatenate/{t.folder_name}", exist_ok=True)
    Lg.logger.info(f"FileScripts:File Name is {video_file_name}")
...
#     os.makedirs(f"/home/sels/Videos/do_concatenate_folder/{t.folder_name}", exist_ok=True)
    ...
    Lg.logger.info(
        f"FileScripts:The duration of the new composite video is {videoclip.duration / 60:.2f} minutes"
    )
    Lg.logger.info(
        f"FileScripts:The new composite video has been renamed to {t.folder_name}.mp4"
    )
    os.chdir(my_work_dir)
    av_end_time = timer()
    av_elapsed_time = av_end_time - av_start_time
    Lg.logger.info(
        f"FileScripts:Execution time to run the sync_av module was {av_elapsed_time/60.:2f} minutes"
    )
    Lg.logger.info(
        "===================================================================="
    )
...
def sort_thumbs():
    ...
    print(os.listdir("."))
    reverse_sorted_files = sorted(os.listdir("."), key=os.path.getctime, reverse=True)
    my_clip = sorted_files + reverse_sorted_files
    Lg.logger.info(f"FileScripts:Sorting process has completed @ {Tm.this_time}")
...
def move_pngs():
    ...
    Lg.logger.info(
        f"FileScripts:Image .png files have been moved to {my_thumbs} @ {Tm.my_date_time}"
    )
...
def move_jpegs():
    ...
    Lg.logger.info(
        f"FileScripts:The jpeg files were successfully moved to {final_thumbs} @ {Tm.my_date_time} ."
    )
...
def copy_video_file(src, dst):
    shutil.copyfile(
        f"{my_full_vids}{t.folder_name}.mp4,{concatenate_candidates}{t.folder_name}.mp4"
    )
...
def move_all():
    ...
    Tm.set_time()
    Lg.logger.info(
        f"FileScripts:Video .avi files have been moved to /Videos/@ {Tm.my_date_time} "
    )
    ...
    Tm.set_time()
    Lg.logger.info(
        f"FileScripts:Video .mp4 files have been moved to /Videos/@ {Tm.my_date_time} "
    )
    ...
    Lg.logger.info(
        f"FileScripts:Video .webm files have been moved to /Videos/@ {Tm.my_date_time} "
    )
    ...
    Lg.logger.info(
        f"FileScripts:Image .png files have been moved to /Thumbs/@ {Tm.my_date_time} "
    )
    ...
    Lg.logger.info(
        f"FileScripts:Image .jpeg files have been moved to /home/sels/Pictures/Mandala Final Thumbs/@ {Tm.my_date_time} "
    )
    Lg.logger.info(
        "================================================================================"
    )
...
def clear_thumbs():
    Lg.logger.info(
        f"FileScripts:Preparing to empty the Thumbs folder @ {Tm.my_date_time} ......."
    )
    shutil.rmtree(my_thumbs)
    os.makedirs(my_thumbs)
    Lg.logger.info(f"FileScripts:Thumbs folder has been emptied @ {Tm.my_date_time} ")
...
def sync_full_vids():
    # Sync Videos
    Lg.logger.info(
        f"FileScripts:Working on sync of MandalaMaker Full Vids folders @ {Tm.my_date_time}"
    )
    source_path_videos = f"{my_home_dir}Videos/Full_Vids/"
    target_path = "/media/Ex_02_Linux/MandalaBackup/Full_Vids"
    target_path_shared_videos = f"{my_shared_drive}/MandalaMaker Full Vids/"
    target_path_usb_ssd_no_audio = "/media/sels/Ex-01-Linux/Mandala Video Library/no_audio/"
    target_path_usb_ssd_full_vids = "/media/sels/Ex-01-Linux/Mandala Video Library/Full_Vids/"
#     target_path_media = (
#         "/mnt/fe9f7833-9c10-449c-bf09-a8dfa3d54adc/Mandala Videos_Backup/"
#     )
    dirsync.sync(source_path_videos, target_path, "sync")
    Lg.logger.info(
        f"FileScripts: Finished Sync of MandalaMaker Full Vids folders to {target_path} completed @ {Tm.this_time}"
    )
    dirsync.sync(source_path_videos, target_path_shared_videos, "sync", create=True)
    Lg.logger.info(
        f"FileScripts: Finished Sync of MandalaMaker Full Vids folders to {target_path_shared_videos} completed @ {Tm.this_time}"
    )
#     dirsync.sync(source_path_videos, target_path_media, "sync", create=True)
#     Lg.logger.info(
#         f"FileScripts: Finished Sync of MandalaMaker Full Vids folders to {target_path_media} completed @ {Tm.this_time}"
#     )
    dirsync.sync(source_path_videos, target_path_usb_ssd_no_audio, "sync", create=True)
    Lg.logger.info(
        f"FileScripts: Finished Sync of MandalaMaker Full Vids folders to {target_path_usb_ssd_no_audio} completed @ {Tm.this_time}"
    )
    dirsync.sync(source_path_videos, target_path_usb_ssd_full_vids, "sync", create=True)
    Lg.logger.info(
        f"FileScripts: Finished Sync of MandalaMaker Full Vids folders to {target_path_usb_ssd_full_vids} @ {Tm.this_time}"
    )
    Lg.logger.info(
        f"FileScripts:Sync of Mandala Video folders completed successfully @ {Tm.this_time}"
    )
...
def sync_module_folders():
    ...
    Tm.set_time()
    Lg.logger.info(
        f"FileScripts: Begin processing sync of MandalaMaker Module folders...."
    )
    source_path_a = my_home_dir + "Modules/"
    target_path_a = f"{my_home_dir}MandalaBackup/Modules/"
    target_path_b = f"{my_home_dir}Git/elemenel/mandala-py/Modules/MandalaMaker/"
    target_path_u = "/media/sels/Ex-01-Linux/Mandala Program Files/"
    target_path_shared_modules = my_shared_drive + "MandalaMaker Modules/"
    dirsync.sync(source_path_a, target_path_a, "sync", create=True)
    Lg.logger.info(
        f"FileScripts: Finished Sync of Modules folders to {target_path_a} @ {Tm.my_date_time}"
    )
    dirsync.sync(source_path_a, target_path_shared_modules, "sync", create=True)
    Lg.logger.info(
        f"FileScripts: Finished Sync of Modules folders to {target_path_shared_modules} @ {Tm.my_date_time}"
    )
    dirsync.sync(source_path_a, target_path_b, "sync", create=True)
    Lg.logger.info(
        f"FileScripts: Finished Sync of Modules folders to {target_path_b} @ {Tm.my_date_time}"
    )
    dirsync.sync(source_path_a, target_path_u, "sync", create=True)
    Lg.logger.info(
        f"FileScripts: Finished Sync of Modules folders to {target_path_u} @ {Tm.my_date_time}"
    )
    Lg.logger.info(
        f"FileScripts:Sync of MandalaMaker Module folders completed successfully @ {Tm.this_time}"
    )
...
def sync_audio_clips():
    ...
    # Sync Audio Clips
    Lg.logger.info(
        f"FileScripts:Begin processing sync of MandalaMaker Audio folders...."
    )
    ...
    target_path_u = "/media/sels/Ex-01-Linux/Mandala Audio Clip Library/"
    dirsync.sync(source_path_audio, target_path_shared_audio, "sync", create=True)
    Lg.logger.info(
        f"FileScripts: Finished  Sync of Audio Clips folders to {target_path_shared_audio} @ {Tm.my_date_time}"
    )
    dirsync.sync(source_path_audio, target_path_back_up_audio, "sync", create=True)
    Lg.logger.info(
        f"FileScripts: Finished Sync of Audio Clips folders to {target_path_back_up_audio} @ {Tm.my_date_time}"
    )
    dirsync.sync(source_path_audio, target_path_u, "sync", create=True)
    Lg.logger.info(
        f"FileScripts: Finished Sync of Audio Clips folders to {target_path_u} @ {Tm.my_date_time}"
    )
    Lg.logger.info(
        f"FileScripts:Sync of MandalaMaker Audio folders completed successfully @ {Tm.this_time}"
    )
...
def sync_images():
    ...
    Lg.logger.info(
        f"FileScripts: Finished Sync of Image folders to {target_path_shared_images} @ {Tm.my_date_time}"
    )
    Lg.logger.info(
        f"FileScripts:Sync of MandalaMaker Image  folders completed successfully @ {Tm.this_time}"
    )
...
def sync_mandala_folders():
    ...
    sync_images()
    Lg.logger.info(
        f"The sync/backup of full_vids, module folders, audio clips and images completed successfully!"
    )
...
# Copies no_audio videos to /home/sels/Videos/concatenate_candidates for processing
def copy_videos(origin,destination):
    #      if my_os == 'Linux':
    try:
        origin = f"{no_audio_vids}"
        destination = "/home/sels/Videos/concatenate_candidates/"
        endswith_ = ".mp4"
        [
            shutil.copy2(os.path.join(origin, i), os.path.join(destination, i))
            for i in os.listdir(origin)
            if i.endswith(endswith_)
        ]
        Lg.logger.info(f"FileScripts:Copying videos @ {Tm.my_date_time}.....")
    except shutil.SameFileError as e:
        pass
    else:
        pass
    Lg.logger.info(f"FileScripts:{no_audio_vids}{t.folder_name} has been copied to conctenate candidates @ {Tm.my_date_time}")
...
def code_backup():
    Lg.logger.info(f"FileScripts:Starting code backup @ {Tm.my_date_time}...........")
    #     shutil.rmtree(my_work_dir +'/Code_Backup/')
    shutil.rmtree(
        "/mnt/fe9f7833-9c10-449c-bf09-a8dfa3d54adc/Modules - MandalaMaker/"
    )  # file:///media/sels/USB DISK/Modules - MandalaMaker
    ...
    Lg.logger.info(
        f"FileScripts:Python Code files have been backed up to MandalaMakerBackup pendrive @ {Tm.my_date_time}"
    )
...
def make_daily_activities_log():
    Tm.set_time()
    Lg.logger.info(f"FileScripts:FileScripts:Creating activity log file @ {Tm.my_date_time}")
    ...
    def create_log_file(module_name):
        ...
        log_file_path = os.path.join(
            os.getcwd(),
            f"{my_home_dir}Modules/MandalaMaker/Daily_Logs, {t.folder_name}.log",
        )
...
def move_thumbs():
    ...
    # Print a message to confirm that the files were moved
    Lg.logger.info(
        f"FileScripts:The Thumb files were successfully moved to the destination directory @ {Tm.my_date_time}."
    )
...
def move_pics():
    ...
    move_jpgs()
    Lg.logger.info(
        f"FileScripts:Image .png files have been moved to /Thumbs/Output/@ {Tm.my_date_time} "
    )
    Lg.logger.info(
        f"FileScripts:Image .jpg files have been moved to /Mandala Final Thumbs/@ {Tm.my_date_time} "
    )
...
#     file = pathlib.Path(f'{my_home_dir}Pictures/Mandalas/')
#     if file.exists ():
#         shutil.rmtree(f'{my_home_dir}Pictures/Mandalas/') #       os.makedirs(f'{my_home_dir}Pictures/Mandalas/')
#     else:
#         os.makedirs(f'{my_home_dir}Pictures/Mandalas/')
...
def update_all():
    ...
    current_path()
    Lg.logger.info(
        f"FileScripts:Pics have been moved to Pictures folder and Dropbox has been updated @ {Tm.my_date_time} "
    )
...
def make_file_folder():
    Lg.logger.info(
        f"FileScripts:Starting make_file_folder() @ {Tm.my_date_time} ..................."
    )
    ...
    Lg.logger.info(
        f"FileScripts:New temp directory /Looped_Pics/Thumbs/{t.folder_name} has been created @ {Tm.my_date_time}"
    )
...
# f'{my_home_dir}Videos/Mandalas/Mystical Mandala by Leon Hatton v.230783807 -r.48002    featuring   680.5    Degree Angles,   with   396 and 852 Hz-FadeBell-7.83sec-42x.mp4' #9.59 minutes
# f'{my_home_dir}Videos/Mandalas/Mystical Mandala by Leon Hatton v.230775214 -r.48002    featuring   408.29999999999995    Degree Angles,   with   Solfeggio_174_bell.mp4' # 9.59 minutes
# f'{my_home_dir}Videos/Mandalas/Mystical Mandala by Leon Hatton v.230763759 -r.48002    featuring   136.1    Degree Angles,   with   396 and 963 Hz-FadeBell-7.83sec-42x.mp4' # 9.59 minutes
...
# F = f'{my_home_dir}Videos/no_audio/Awesome Mandala-(7)1124, 2248, 3372, 4496, 5620, 6744, 7868 angles-Audio-396Hz-fadebell-18min-232755617.mp4'
G = f"{my_home_dir}Videos/no_audio/Random Mandalas-(4)135, 270, 405, 675 degrees angles-Audio-F Project- Crown Chakra Tones-1726, 344.12, 688.24, 1376.48-36min-232825430.mp4"
# Long Duration Audio Clips
# f'{my_home_dir}Music/Solfeggio Tones/174hz-bell-6sec-fade-240x.mp3'
# f'{my_home_dir}Music/Solfeggio Tones/285 Hz-bell-6sec-fade-240x.mp3'
# # Z = f'{my_home_dir}Music/Solfeggio Tones/396 Hz-bell-6sec-fade-240x.mp3'
# # Y = f'{my_home_dir}Music/Solfeggio Tones/417 Hz-bell-6sec-fade-240x.mp3'
# M = f'{my_home_dir}Music/Solfeggio Tones/639 Hz-bell-6sec-fade-240x.mp3'
# f'{my_home_dir}Music/Solfeggio Tones/741 Hz-bell-6sec240x.mp3'
# f'{my_home_dir}Music/Solfeggio Tones/852 Hz-bell-6sec240x.mp3'
# f'{my_home_dir}Music/Solfeggio Tones/963 Hz-bell-6sec240x.mp3'
# H = f'{my_home_dir}Music/Solfeggio Tones/Solfeggio_528_bell_6sec-fade-240x.mp3'
# M = f'{my_home_dir}Music/Audio Clips for Python/Earth Tones/194.18-388.36-1165.08-earthtones-6secbellx180.mp3'
N = f"{my_home_dir}Music/Audio Clips for Python/SevenChakraTones-144-432-528-639-741-852-1296 Hz.mp3"
...
def make_my_movie():
    ...
    folder_name = f"{my_home_dir}Videos/Special/{my_title}"
    ...
    my_audio = N  # H #Y #Z # f'{my_home_dir}Music/Solfeggio Tones/Solfeggio_528  Hz_bell_6sec_*120.mp3'
    Lg.logger.info(f"FileScripts:Starting make_my_movie() @ {Tm.this_time}")
    os.chdir(f"{folder_name}")
    new_video = "__temp__.mp4"
    videoclip = VideoFileClip(my_video)
    Lg.logger.info(
        f"FileScripts:The duration of this video clip is {videoclip.duration / 60:.2f} minutes"
    )
    ...
    Lg.logger.info(
        f"FileScripts:The duration of this new video is {videoclip.duration / 60:.2f} minutes"
    )
    Lg.logger.info(f"FileScripts:This new video has been renamed to {my_title}.mp4")
    Lg.logger.info(
        f"FileScripts:The running of this module, make_my_movie, has successfully completed @ {Tm.this_time}"
    )
    os.chdir(my_work_dir)
    time.sleep(3)
    Lg.logger.info(
...
def copy_pics():
    for num in range(300):
        src = (
            my_work_dir
            + "Images/An Awesome Polygram Mandala featuring 1512  Degree Angles_999_.jpg"
        )
        dest = (
            my_work_dir
            + "An Awesome Polygram Mandala featuring 1512  Degree Angles"
            + str(num)
            + ".jpg"
        )
...
def copy_mp3s():
    if my_os == "Linux":
        try:
            origin = rf"{my_home_dir}Music/ripped/wav//"
            destination = rf"{my_home_dir}Music/truncated//"
            endswith_ = ".mp3"
            [
                shutil.copy(os.path.join(origin, i), os.path.join(destination, i))
                for i in os.listdir(origin)
            ...
            Lg.logger.info(
                f"FileScripts:Copying music from ripped/wav to truncated....."
            )
        except shutil.SameFileError as e:
            pass
    else:
...
def get_video_duration():
    ...
    for filename in Path(my_work_dir).glob("*.mp4"):
        ...
        Lg.logger.info(
            str(filename) + ": " + str(round(clip.duration / 60)) + " minutes"
        )
...
def purge_thumbs():
    ...
    for png_file in png_files:
        ...
        if (
            now - file_creation_time > 1
        ):  # 604800: # use 1 for immediate run; #604800:  # 1 week in seconds; default
            # Delete the file
